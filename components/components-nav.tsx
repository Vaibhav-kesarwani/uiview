...
export interface ComponentsNavProps {
  items: SidebarNavItem[];
}

export function ComponentsNav({ items }: ComponentsNavProps) {
    ...
    <div className="flex flex-col gap-6">
      {items.map((item, index) => (
        <div key={index} className="flex flex-col gap-1">
          ...
          {item?.items?.length && (
            <ComponentsNavItems items={item.items} pathname={pathname} />
          )}
        </div>
      ))}
    </div>
...
interface ComponentsNavItemsProps {
  items: SidebarNavItem[];
  pathname: string | null;
}

export function ComponentsNavItems({
  items,
  pathname,
}: ComponentsNavItemsProps) {
  return items?.length ? (
    <div className="relative grid grid-flow-row auto-rows-max gap-0.5 text-sm">
      {items.map((item, index) =>
        item.href && !item.disabled ? (
          ...
          >
            {/* 🟡 Updated text color for the nav item title */}
